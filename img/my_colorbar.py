...
# made by Menaka
# colorbars defined for Revel et al 2020, 2023 etc..
...
def diverging_colormap(colorL='#052f61',colorR='#67001f'):
    """
    Create diverging colormap by defining each color at left and right of the divergent point
    """
    nodes = [0.0, 0.5, 1.0] 
    ...
    return cmap
###################
def multiple_div_colormap(colorlistL,colorlistR):
    #=== L and R should be similar
    # provide two set of colors for making a divergence colormaps
    if len(colorlistL) != len(colorlistR):
        return "Left and Right color list are different"
    else:
        step=0.5/float(len(colorlistL))
        nodes = np.linspace(0.0,1.0,2*len(colorlistL)) 
        colorlist = colorlistL #[colorlist,'white',colorlist] # Left color _ white _ Righ color
        # colorlist.extend(['white'])
        colorlist.extend(colorlistR)
        cmap = LinearSegmentedColormap.from_list("", list(zip(nodes, colorlist)))
    return cmap
###################