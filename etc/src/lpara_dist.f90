...
call getarg(8,buf)
read(buf,*) threshold ! threshold for defining the local patch, distance [e.g., 1000km]

call getarg(9,buf)
read(buf,*) thrname ! threshold name for defining the local patch

call getarg(10,buf)   ! radius of search area
read(buf,*) patch_size

call getarg(11,buf)   ! to represent dams or not
read(buf,*) dam
...
do ix = 1, nx ! pixels along longtitude direction
    do iy = 1, ny ! pixel along latitude direction
        lat = north-(iy-1.0)*gsize
        lon = west+(ix-1.0)*gsize
        !remove ocean
        if (ocean(ix,iy)==1) then
            cycle
        end if
        print*, "===",lat,lon,"===",ix,iy,rivwth(ix,iy)
        ...
        print*, "read gausssian weight",fname 
        fn = 34
        ...
        do j=iy-patch_size,iy+patch_size
            do i=ix-patch_size,ix+patch_size !bugfix on 2021/08/23
                ...
                if (weightage(i_m,j_m) < 0.01) then
                ...
                ! lag_dist <= local patch
                if (lag_dist > threshold) then ! thershold is a distance [e.g., 1000km]
                    cycle
                end if
                ! write to the patch file
                ! print*, ix,iy,i_m,j_m,gauss_weight(i_m,j_m),weightage(i_m,j_m), lag_dist
                write(fn,21)i_m,j_m,gauss_weight(i_m,j_m)
                write(*,21)i_m,j_m,gauss_weight(i_m,j_m)
                print*, weightage(i_m,j_m)
                ! print*, i_m,j_m,gauss_weight(i_m,j_m),weightage(i_m,j_m),lag_dist
                ! find the target pixel
                ! save the target pixel
...
! calculate the distance between two river grids along the river
implicit none 
!--
...
if (ud==-1) then
  ...
  do while (ix/=tx .or. iy/=ty) 
    ...
    !print*, nextdst(ix,iy)
    rl=anint((nextdst(ix,iy)/1000.0)*100)/100.0 ! km
